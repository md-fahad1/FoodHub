...
import OrderTable from "@/components/Dashboard/Order/UserOrderTable";
import UserOrderTable from "@/components/Dashboard/Order/UserOrderTable";
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_ENDPOINT}/Order/user/${currentUser?.Id}`
        );
    ...
    <div>
      ...
      <div className="overflow-auto w-[95%] mx-auto mt-6 border rounded-md h-full shadow-[0_0px_5px_0px_rgba(0,0,0,0.3)]">
        <div className="bg-white min-w-[600px]">
          <UserOrderTable productsData={products} />
        </div>
      </div>
    </div>
  );
